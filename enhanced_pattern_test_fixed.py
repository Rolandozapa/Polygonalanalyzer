...
class EnhancedPatternDetectionTestSuite:
    ...
    def get_api_data(self, endpoint):
        """Helper method to get API data with proper format handling"""
        try:
            ...
            if isinstance(data, dict):
                ...
                elif f"{endpoint}" in data:
                    return data[f"{endpoint}"], None
                elif 'analyses' in data and endpoint == 'analyses':
                    return data['analyses'], None
                elif 'opportunities' in data and endpoint == 'opportunities':
                    return data['opportunities'], None
                elif 'decisions' in data and endpoint == 'decisions':
                    return data['decisions'], None
                else:
                    # Return the data as-is if it's a dict but doesn't match expected format
                    return data, None
            else:
                return data, None
    ...
    async def test_new_pattern_types(self):
        ...
        try:
            analyses, error = self.get_api_data('analyses')
            
            if error:
                self.log_test_result("New Pattern Types Detection", False, error)
                return
            ...
            total_analyses = len(analyses) if isinstance(analyses, list) else 1
            
            # Handle both list and single item responses
            analyses_list = analyses if isinstance(analyses, list) else [analyses]
            
            for analysis in analyses_list[:20]:  # Check last 20 analyses
            ...
            success = len(new_patterns_found) >= 1  # At least 1 new pattern type should be detected
    ...
    async def test_advanced_patterns(self):
        ...
        try:
            ...
            if error:
                self.log_test_result("Advanced Patterns Detection", False, error)
                return
            ...
            for analysis in analyses_list[:25]:  # Check last 25 analyses
                symbol = analysis.get('symbol', 'UNKNOWN')
                patterns_detected = analysis.get('patterns_detected', [])
            ...
            success = len(advanced_patterns_found) >= 1 or harmonic_patterns_found >= 1
    ...
    async def test_pattern_quality(self):
        ...
        try:
            ...
            if error:
                self.log_test_result("Pattern Quality Verification", False, error)
                return
            ...
            for analysis in analyses_list[:15]:  # Check last 15 analyses
                symbol = analysis.get('symbol', 'UNKNOWN')
            ...
            if confidence_scores:
                ...
                success = (
                    reasonable_confidences >= len(confidence_scores) * 0.5 and  # 50% should be in 0.7-0.9 range
    ...
    async def test_system_integration(self):
        ...
        try:
            opportunities, opp_error = self.get_api_data('opportunities')
            analyses, ana_error = self.get_api_data('analyses')
            
            if opp_error:
                self.log_test_result("System Integration", False, f"Opportunities API error: {opp_error}")
                return
                
            if ana_error:
                self.log_test_result("System Integration", False, f"Analyses API error: {ana_error}")
                return
                
            # Check pattern filtering and deduplication
            total_opportunities = len(opportunities) if isinstance(opportunities, list) else 1
            total_analyses = len(analyses) if isinstance(analyses, list) else 1
            ...
            for analysis in analyses_list[:10]:
                symbol = analysis.get('symbol', 'UNKNOWN')
            ...
            analyses_with_patterns = sum(1 for analysis in analyses_list if analysis.get('patterns_detected', []))
    ...
    async def test_backend_logs(self):
        ...
        try:
            ...
            success = (
                len(strong_pattern_logs) > 0 and  # Should have some pattern detection activity
                no_import_errors  # No import errors
            )
    ...
    async def test_pattern_variety_multiple_symbols(self):
        ...
        try:
            ...
            if error:
                self.log_test_result("Pattern Variety Test", False, error)
                return
            ...
            for analysis in analyses_list[:30]:  # Check last 30 analyses
                symbol = analysis.get('symbol', 'UNKNOWN')
                patterns_detected = analysis.get('patterns_detected', [])
                confidence = analysis.get('analysis_confidence', 0)
            ...
            success = (
                len(unique_symbols) >= 1 and  # At least 1 different symbol
                len(all_patterns_found) >= 3  # At least 3 different pattern types
            )
