    ...
    async def analyze_opportunity(self, opportunity: MarketOpportunity) -> Optional[TechnicalAnalysis]:
        ...
        try:
            ...
            {{
                ...
                "confidence": 0.75,
                "reasoning": "REGIME ANALYSIS: Market regime is {regime} with {confidence:.1%} ML confidence (Technical consistency: {technical_consistency:.1%}). STEP 1 - RSI MOMENTUM: RSI 14-period is {rsi:.1f} in {rsi_zone} zone, suggesting {rsi_interpretation}. STEP 2 - MACD TREND: MACD histogram {macd_histogram:.6f} shows {macd_trend} momentum with {macd_direction} directional bias. STEP 3 - ADX STRENGTH: ADX {adx:.1f} indicates {adx_strength} trend strength using Wilder method (+DI: {plus_di:.1f}, -DI: {minus_di:.1f}). STEP 4 - MFI VOLUME: Money Flow Index {mfi:.1f} shows {mfi_signal} institutional activity. STEP 5 - BOLLINGER ANALYSIS: Price at {bb_position:.1%} of BB range, squeeze status: {bb_squeeze} ({squeeze_intensity}). STEP 6 - VWAP POSITION: Price is {vwap_position:+.1f}% from VWAP indicating {vwap_strength}. STEP 7 - CONFLUENCE GRADE: Setup grades {confluence_grade} with {confluence_score} points - {conviction_level} conviction level. CONCLUSION: [Final decision based on regime + confluence analysis].",
