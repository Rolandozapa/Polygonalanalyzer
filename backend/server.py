...
"""
    ...
    async def analyze_opportunity(self, opportunity: MarketOpportunity) -> Optional[TechnicalAnalysis]:
        ...
        try:
            ...
            rsi = simple_indicators_data.get('rsi', 50.0)
            macd_signal = simple_indicators_data.get('macd_signal', 0.0)
            macd_line = simple_indicators_data.get('macd_line', 0.0)
            macd_histogram = simple_indicators_data.get('macd_histogram', 0.0)
            ...
            atr = simple_indicators_data.get('atr', 0.01)
            vwap = simple_indicators_data.get('vwap', opportunity.current_price)
            vwap_position = simple_indicators_data.get('vwap_distance', 0.0)  # ✅ Use VWAP distance instead of MFI
            volume_ratio = simple_indicators_data.get('volume_ratio', 1.0)
            volume_trend = simple_indicators_data.get('volume_trend', 'NEUTRAL')
            volume_surge = simple_indicators_data.get('volume_surge', False)
            ...
            {{
                ...
                "confidence": 0.75,
                "reasoning": "STEP 1 - RSI ANALYSIS: RSI is {rsi:.1f} which is [oversold/neutral/overbought] suggesting [interpretation]. STEP 2 - MACD MOMENTUM: MACD histogram {macd_histogram:.6f} shows [bullish/bearish/neutral] momentum because [explanation]. STEP 3 - MFI INSTITUTIONAL: MFI {mfi:.1f} indicates [accumulation/distribution/neutral] with institutional money [flowing in/out/neutral]. STEP 4 - VWAP POSITION: Price is {vwap_position:+.1f}% from VWAP suggesting [price strength/weakness]. STEP 5 - EMA TREND: {trend_hierarchy} hierarchy confirms [trend direction]. STEP 6 - PATTERN CONFLUENCE: Detected patterns {detected_pattern_names} [support/contradict] the technical indicators. CONCLUSION: [Final decision based on confluence analysis].",
