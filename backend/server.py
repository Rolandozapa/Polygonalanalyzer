...
"""
    ...
    async def analyze_opportunity(self, opportunity: MarketOpportunity) -> Optional[TechnicalAnalysis]:
        ...
        try:
            ...
            # ÉTAPE 1: Récupération données 15m (pour compatibilité avec le code existant)
            ...
            # 🎯 ÉTAPE 1.5: ANALYSE MULTI-TIMEFRAME (En parallèle pour enrichissement IA1)
            ...
            try:
                ...
                logger.error(f"❌ Multi-timeframe analysis failed for {opportunity.symbol}: {e}")
            ...
            if historical_data is None or len(historical_data) < min_periods:
                logger.warning(f"⚠️ DONNÉES INSUFFISANTES: {opportunity.symbol} - {len(historical_data) if historical_data is not None else 0} périodes 15m (min: {min_periods} pour MACD)")
                return None
            ...
            prompt_variables = {
                ...
                # ❌ Aroon, SAR, MFI supprimés (redondants avec ADX/Volume Ratio)
                
                # Volume and flow (simplified)
            ...
            logger.info(f"✅ IA1 externalized prompt loaded and formatted for {opportunity.symbol}")
            ...
            analysis_data.update({
                ...
                # 🎯 FIGURES CHARTISTES (remplace indicateurs redondants)
                "detected_patterns": detected_pattern_names[:3] if detected_pattern_names else ["Aucune"],
                "primary_pattern": detected_pattern_names[0] if detected_pattern_names else "Aucune",
                "pattern_strength": all_detected_patterns[0].strength if all_detected_patterns else 0,
                "pattern_direction": all_detected_patterns[0].trading_direction if all_detected_patterns else "NEUTRAL",
    ...
    async def _fetch_bingx_intraday_data(self, symbol: str, timeframe: str, days: int) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Format symbole pour BingX API (ETHUSDT -> ETH-USDT)
            if 'USDT' in symbol:
    ...
    def _validate_analysis_data(self, analysis_data: dict) -> dict:
        ...
        try:
            ...
            logger.info(f"🔥 ADVANCED INDICATORS VALIDÉS: VWAP={vwap_val_disp} ({cleaned_data['vwap_signal']}), EMA={cleaned_data['ema_hierarchy']} ({ema_strength_disp})")
            ...
            return {
                ...
                "plus_di": 25.0,
                "minus_di": 25.0,
                "detected_patterns": ["Aucune"],
                "primary_pattern": "Aucune",
                "pattern_strength": 0.0,
                "pattern_direction": "NEUTRAL",
                "mtf_primary_signal": "NEUTRAL",
                "mtf_confluence_score": 0.5,
                "mtf_confidence_grade": "C",
                "mtf_recommended_action": "HOLD",
                "mtf_risk_reward_ratio": 1.0,
                "tf_15m_signal": "NEUTRAL",
                "tf_1h_signal": "NEUTRAL",
                "tf_4h_signal": "NEUTRAL",
                "tf_1d_signal": "NEUTRAL",
                "vwap_distance": 0.0,
                "volume_ratio": 1.0,
...
# Global instances
    ...
    def _calculate_enhanced_institutional_validation(self, vwap_position: float, vwap_price: float, 
        ...
        try:
            ...
            base_score = 65.0  # Start with slightly higher base score (no MFI penalty)
                ...
                if await orchestrator._should_send_to_ia2(analysis, target_opportunity):
                    ...
                    if decision:
                        ...
                        return {
                            ...
                            "analysis": analysis.dict(),  # Retourner l'objet analysis complet
                            "ia2_decision": {
...
async def automatic_scout_refresh_loop():
    ...
    import os
    refresh_interval = int(os.environ.get('SCOUT_REFRESH_INTERVAL', '14400'))  # Production: 4 hours (14400 seconds)
